...
const makeRobot = (state: State, blueprint: Blueprint, type: number) => {
  ...
  temp.inventory = temp.inventory.map((i, ii) => i - cost[ii] + temp.bots[ii]);
  if (type === GEODE_ROBOT) {
...
const solve = (fn: string) => {
  ...
  for (let i = 0; i < blueprints.length; i++) {
    const score = findMaxGeodes(initialState(24), blueprints[i], [16, 6, 3, 2]);
    p1 = p1 + score * (i + 1);
  }
...
const solveMe = () => {
  // const data = fs.readFileSync("/src/day19/input.txt", "utf-8");
  const result = solve("day19");
  console.log("JOE: result: ", result);
};
